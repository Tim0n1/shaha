...
Note that before running this module requires the rendered dataset to be downloaded and split (see the :mod:`intellichess.data_synthesis` module for more information).

Note that script is different to :mod:`intellichess.core.piece_classifier.create_dataset` because the squares are cropped differently.

.. code-block:: console

    $ python -m intellichess.piece_classifier.create_dataet --help p
...
from intellichess.core import sort_corner_points
from intellichess.core.dataset import piece_name
...
def warp_chessboard_image(img: np.ndarray, corners: np.ndarray) -> np.ndarray:
    ...
    Note: this method is different to :meth:`intellichess.core.piece_classifier.create_dataset.warp_chessboard_image` because the the warped image will be bigger.
